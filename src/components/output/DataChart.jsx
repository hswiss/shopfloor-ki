    ...
    <ResponsiveContainer width="100%" height={Math.max(300, data.length * 52)}>
      <BarChart data={data} layout="vertical" margin={{ left: 0, right: 50, top: 0, bottom: 0 }}>
        ...
        <YAxis
          ...
          tickLine={false}
        />
        <Bar dataKey="pct" radius={[0, 6, 6, 0]} barSize={30} label={RenderBarLabel}>
          {data.map((entry) => (
...
function CustomYTick({ x, y, payload }) {
  ...
  return (
    <text x={x} y={y} dy={4} textAnchor="end" fill="#a1a1aa" fontSize={13}>
      {display}
    </text>
  );
}
...
function RenderBarLabel({ x, y, width, value }) {
  return (
    <text x={x + width + 6} y={y + 19} fill="#e4e4e7" fontSize={14} fontWeight={700}>
      {value.toFixed(1)}%
    </text>
  );
}
        ...
        <tbody>
          {data.map((row) => (
            <tr key={row.name} className="border-b border-zinc-800/50">
              ...
              <td className="py-2 text-right text-zinc-300 font-semibold">{row.pct.toFixed(1)}%</td>
            </tr>
          ))}
        </tbody>
      </table>
...
export default function DataChart({ percentages, totals, format, onFormatChange }) {
  ...
  return (
    <div>
      <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-4 min-h-[300px]">
